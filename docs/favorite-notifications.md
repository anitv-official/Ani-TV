...
- `appwrite-functions/anitv-notifications/src/adapters/manga-swat.js`: محول Manga Swat لروابط Favorites من نوع `swat://series/{id}/{slug}`. يتابع صفحات `series/{id}/chapters/`، يختار أعلى رقم فصل لا ترتيب API، ويستخدم `swat:chapter:{id}` كمفتاح إصدار ثابت.
