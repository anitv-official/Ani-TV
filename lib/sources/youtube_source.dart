...
class YoutubeSource extends ContentSource {
  ...
  @override Future<List<Map<String, dynamic>>> nextPage({String query = '', int page = 2}) async {
    ...
    final response = await _client.post(Uri.https('www.youtube.com', '/youtubei/v1/search', {'prettyPrint': 'false'}), headers: const {'User-Agent': _userAgent, 'Accept-Language': 'ar,en;q=0.8', 'Content-Type': 'application/json'}, body: jsonEncode({'context': {'client': {'clientName': 'WEB', 'clientVersion': '2.20250915.01.00'}}, 'continuation': token})).timeout(const Duration(seconds: 30));
  ...
  Future<List<Map<String, dynamic>>> _searchPage(String query, int page) async {
    final value = query.trim().isEmpty ? 'أحدث فيديوهات عربية' : query.trim();
    final uri = Uri.https('www.youtube.com', '/results', {'search_query': value, if (page > 1) 'page': '$page'});
  ...
  void _walk(dynamic value, List<Map<String, dynamic>> output) {
    ...
    final renderer = value['videoRenderer'] ?? value['compactVideoRenderer'];
    if (renderer is Map) {
      final id = renderer['videoId']?.toString() ?? '';
      if (id.isNotEmpty && output.every((item) => item['video_id'] != id)) {
        final title = _runs(renderer['title']);
        final channel = _runs(renderer['ownerText'] ?? renderer['longBylineText']);
  ...
  String? _findContinuation(dynamic value) {
    if (value is List) {
      for (final child in value) {
        final found = _findContinuation(child);
