...
class _YouTubeCatalogSectionState extends State<YouTubeCatalogSection> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      TextButton.icon(onPressed: loadingMore || items.length >= allItems.length ? null : () => _load(more: true), icon: loadingMore ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.expand_more), label: const Text('تحميل المزيد من YouTube')),
    ]);
  }
}
