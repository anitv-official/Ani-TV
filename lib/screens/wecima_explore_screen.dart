...
class _WecimaDetailsScreenState extends State<WecimaDetailsScreen> {
  ...
  Future<void> _play(String url, String title) async {
    try {
      ...
      final servers = (data['direct_stream_urls'] as List? ?? const []).whereType<Map>().map((x) => {'url': x['url']?.toString() ?? '', 'label': x['label']?.toString() ?? x['name']?.toString() ?? 'سيرفر'}).where((x) => x['url']!.isNotEmpty).toList();
  ...
  void _openPlayer(Map<String, String> server, String title) => Navigator.push(context, MaterialPageRoute(builder: (_) => VideoPlayerScreen(url: server['url']!, title: title, episodeId: widget.url, directStreamUrls: [server])));
