...
function catalogUrl(value) {
  try {
    ...
    if (parts[0] === 'shows' && parts.length >= 3 && !/^(?:genre|season|seasons)$/i.test(parts[1])) {
      return `${u.origin}/shows/${parts[1]}`;
    }
    return value;
...
function itemFrom(block, base) { const link = block.match(/href=["']([^"']+)["']/i); if (!link) return null; const originalUrl = abs(link[1], base); if (!isCatalogItemUrl(originalUrl)) return null; const url = catalogUrl(originalUrl); const series = new URL(url).pathname.startsWith('/shows/'); return { title: titleFrom(block, url).replace(/\s*—\s*الحلقة\s*\d+$/i, '').trim(), url, image_url: imageFrom(block, base), type: series ? 'مسلسل' : 'فيلم', category: 'wecima', source: 'Wecima', source_id: 'wecima' }; }
...
function isAllowedPlayerUrl(value) { try { const host = new URL(value).hostname.toLowerCase().replace(/^www\./, ''); return ['streamtape.cc','luluvdo.com','uqload.net','uqload.vc','streamwish.to','streamwish.fun','topcinemaa.com','topcinemaa.cc','topcinemaa.co','web2.topcinemaa.com'].some(x => host === x || host.endsWith(`.${x}`)); } catch (_) { return false; } }
...
module.exports = async function handler(req, res) { res.setHeader('Access-Control-Allow-Origin', '*'); res.setHeader('Cache-Control', 'public, max-age=30'); try { const { action = 'latest', page = '1', q = '', url = '' } = req.query; if (action === 'latest' || action === 'search') { const target = action === 'search' && q ? `${BASES[0]}/?s=${encodeURIComponent(q)}` : (Number(page) <= 1 ? `${BASES[0]}/` : `${BASES[0]}/page/${Math.max(1, Number(page))}/`); const result = await get(target); const items = catalogItems(result.html, result.base); return res.json({ items, page: Number(page), hasMore: items.length > 0 }); } if (!url || !BASES.some(b => url.startsWith(b))) return res.status(400).json({ error: 'invalid url' }); const result = await get(url); if (action === 'servers') return res.json({ url, servers: extractServers(result.html) });
    const parsed = parseDetails(result.html, url, result.base);
    try {
      const path = new URL(url).pathname.replace(/\/+$/, '');
      if (/^\/shows\/[^/]+$/i.test(path)) {
        const seasonUrls = [...new Set([...result.html.matchAll(/href=[\"']([^\"']+\/shows\/[^\"']+\/seasons\/[^\"']+)[\"']/gi)].map((m) => abs(m[1], result.base)))];
        for (const seasonUrl of seasonUrls.slice(0, 20)) {
          try { const season = await get(seasonUrl); parsed.episodes.push(...parseDetails(season.html, seasonUrl, season.base).episodes); } catch (_) {}
        }
        parsed.episodes = [...new Map(parsed.episodes.map((episode) => [episode.url, episode])).values()].sort((a, b) => a.number - b.number || a.url.localeCompare(b.url));
      }
    } catch (_) {}
    return res.json(parsed); } catch (e) { return res.status(502).json({ error: 'wecima bridge unavailable', message: e.message }); } };
module.exports._test = { isCatalogItemUrl, catalogUrl, directMediaUrl, isAllowedPlayerUrl, catalogItems, extractServers, parseDetails };
