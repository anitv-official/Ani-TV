...
## إصلاح تحديث Profile والصورة — 2026-09-13

تمت إضافة استرداد تلقائي لـProfile ID من Appwrite عند محاولة تعديل Username أو صورة الملف الشخصي إذا لم يكن المعرف محمّلًا في الذاكرة، مع الاعتماد على وجود User ID من الجلسة بدل ربط العملية بحالة توثيق البريد فقط. أصبحت عملية الصورة تنشئ/تتحقق من Profile أولًا، ثم ترفع الصورة، ثم تربط File ID بالصف؛ وإذا فشل الربط بعد الرفع يُحذف الملف الجديد لتجنب الملفات اليتيمة ويظل الملف القديم محفوظًا. كما أصبحت بطاقة Profile تعرض `birthdate` و`country` القادمين من Profiles Table.
