...
## تدفق التسجيل النهائي — 2026-09-13

يقبل التسجيل Username الذي يدخله المستخدم فقط، بينما ينشئ Appwrite User ID تلقائيًا باستخدام `ID.unique()`. يتم فحص Username بعد التطبيع، ثم يُنشأ Profile تلقائيًا بالـUser ID الناتج والحقول المؤكدة فقط. يستخدم إنشاء الصف معرف Username المطبع مع Unique Index لمنع السباقات والتكرار، ويحوّل تعارض `409` إلى Username مأخوذ. إذا فشلت خطوة Profile بعد إنشاء الحساب، تُغلق جلسة الحساب الجديدة ويُحافظ على User لاستكمال الاسترداد بدل ترك جلسة نصف مكتملة أو إنشاء User مكرر.


## تخزين Profile السحابي وتغيير Username — 2026-09-13

تمت مواءمة الكود مع Attributes الجديدة بأسمائها الحرفية lowercase: `displayname`, `birthdate`, و`country`. تُرسل هذه الحقول عند إنشاء Profile، وتُقرأ عند استعادة الجلسة، ويُحدّث `displayname` عند تعديل الاسم الظاهر. لا يُرسل `userId` في أي تحديث، ولذلك يبقى ثابتًا ولا يمكن تغييره عبر مسار تعديل Username أو Profile.

تغيير Username يحدّث الصف نفسه بدل إنشاء صف جديد. يفحص الكود الاسم بعد التطبيع ويستثني صف المستخدم الحالي من فحص التوفر، ثم يرسل الاسم الجديد إلى الصف. بعد نجاح التحديث يختفي الاسم القديم من Profiles فيصبح متاحًا، بينما يبقى الاسم الجديد محجوزًا عبر Unique Index. تعارض السباق `409` يعامل كاسم مستخدم مأخوذ.


## إصلاح فحص توفر Username — 2026-09-13

كانت شاشة التسجيل تعرض «تعذر التحقق من اسم المستخدم» لأن `_listProfileRows()` كان يرسل استعلام TablesDB بصيغة `limit(5000)`. تحقق الاختبار المباشر من أن endpoint الحالي يعيد `400 general_query_invalid` عند إرسال هذا الاستعلام، بينما يعيد `200` مع `total` و`rows` عند طلب الصفوف دون هذا الاستعلام. أزيلت الصيغة غير المدعومة، مع إبقاء التطبيع والمقارنة ومنع التكرار كما هي.
